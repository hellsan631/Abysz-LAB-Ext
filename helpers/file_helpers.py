...
def copy_rename_images(
    ...
    for i, file_name in tqdm(enumerate(file_list), total=len(file_list), desc=f"Copy-Renaming Source Images with {filename_format} format"):
        ...
        new_file_name = filename_format.format(i+1) # new file name with format %04d
        new_path = os.path.join(frames_folder, new_file_name + os.path.splitext(file_name)[1]) # nueva ruta del archivo
        try:
            os.rename(old_path, new_path)
        except FileExistsError:
            print(f"File {new_file_name} already exists. Rename skipped.")

# The first stage adds a temporary unique identifier ("rename") to avoid conflicts if the directory already contains files with names like the 
# intended new names (e.g., 0001.ext). The second stage removes this identifier, leaving the files with clean, sequentially numbered names.
def sequential_generated_rename(generated_frames_folder):
    copy_rename_images(generated_frames_folder, "{:04d}rename")
    copy_rename_images(generated_frames_folder, "{:04d}")
